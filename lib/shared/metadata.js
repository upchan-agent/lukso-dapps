...
/**
 * Build LSP4 metadata (legacy signature for backward compatibility)
 * @deprecated Use createLSP4Metadata({ name, description, ... }) instead
 */
export function createLSP4MetadataLegacy(title, description, imageCid = null, videoCid = null, audioCid = null, audioExt = null, tags = []) {
  const images = imageCid ? [[{ url: `ipfs://${imageCid}` }]] : [];
  const assets = [];
  
  if (videoCid) {
    assets.push({ url: `ipfs://${videoCid}`, fileType: 'video/mp4' });
  }
  if (audioCid) {
    assets.push({ url: `ipfs://${audioCid}`, fileType: audioExt || 'audio/mpeg' });
  }

  return createLSP4Metadata({
    name: title,
    description,
    images,
    assets,
