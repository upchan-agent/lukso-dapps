 ...
 * - Relay: value is passed via msg.value and signed in the LSP25 digest (per LSP25 spec).
 * - Direct: Controller calls UP.execute() directly (matches official implementation).
 * 
 ...
 * @param {bigint|number} params.value - LYX amount to forward via msg.value (default: 0)
 * @returns {Promise<{signature, nonce, kmAddress, chainConfig}>}
 */
async function _buildRelaySignature({ upAddress, controllerAddress, privateKey, payload, network, value = 0 }) {
  ...
  // value = native token amount to forward alongside the call (per LSP25 spec)
  const valueBigInt = typeof value === 'bigint' ? value : BigInt(value);
  const encodedMessage = ethers.solidityPacked(
    ['uint256', 'uint256', 'uint256', 'uint256', 'uint256', 'bytes'],
    [LSP25_VERSION, BigInt(chainConfig.chainId), nonce, 0n, valueBigInt, payload]
  );
 ...
 * @param {bigint|number} params.value - LYX amount to forward (default: 0)
 * @returns {Promise<{transactionHash: string, explorerUrl: string}>}
 */
async function _sendRelayRequest({ upAddress, payload, signature, nonce, chainConfig, value = 0 }) {
  const valueBigInt = typeof value === 'bigint' ? value : BigInt(value);
  const relayRequest = {
    address: upAddress,
    transaction: {
      ...
      value: ethers.toBeHex(valueBigInt),
    }
  };
 ...
 */
export async function executeRelay(upAddress, controllerAddress, privateKey, payload, network = DEFAULT_NETWORK, value = 0) {
  const { signature, nonce, chainConfig } = await _buildRelaySignature({
    upAddress, controllerAddress, privateKey, payload, network, value
  });

  return _sendRelayRequest({ upAddress, payload, signature, nonce, chainConfig, value });
}
...
export async function executeDirect(payload, privateKey, upAddress, network = DEFAULT_NETWORK, value = 0) {
  ...
  // LSP6 model (LUKSO) — direct UP.execute() call (matches official implementation)
  const up = new ethers.Contract(upAddress, ABIS.LSP0, wallet);
  const valueBigInt = typeof value === 'bigint' ? value : BigInt(value);

  // Decode the UP.execute(operation, target, value, data) payload
  const upInterface = new ethers.Interface(ABIS.LSP0);
  const decoded = upInterface.decodeFunctionData('execute', payload);
  const tx = await up.execute(decoded.operation, decoded.target, decoded.value, decoded.data, { value: valueBigInt });
  const receipt = await tx.wait();
  return {
    transactionHash: receipt.hash,
    explorerUrl: `${chainConfig.explorerUrl}/tx/${receipt.hash}`
...
export async function executeWithFallback(options) {
  ...
  try {
    ...
    const relayResult = await executeRelay(upAddress, controllerAddress, privateKey, payload, network, value);
    if (process.env.DEBUG_LSP25) {
...
export async function checkQuota(upAddress, privateKey, network = DEFAULT_NETWORK) {
  ...
  if (process.env.DEBUG_LSP25) {
    console.log('[DEBUG] Quota check:', { upAddress, timestamp, messageHash });
  }
 ...
 */
export function buildExecuteCall(targetAddress, calldata, value = 0n, operation = 0) {
  const upInterface = new ethers.Interface(ABIS.LSP0);
...
export async function executeLSP7Transfer(options) {
  ...
  // Wrap in UP.execute()
  const payload = buildExecuteCall(tokenAddress, calldata);
...
export async function executeLyxTransfer(options) {
  ...
  const payload = buildExecuteCall(recipient, '0x', amountBigInt);
